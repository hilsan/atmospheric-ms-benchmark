#### 2025-09-09 I have an issue with the code for fused rings, so they should be double checked
import pandas as pd
...
def plot_number_of_molecules_with_substitutions(df):
    ...
    sns.despine(left=True, bottom=True)  # Remove top/right spines
    ax.set_facecolor('white')

    # Font settings
    modern_font = "DejaVu Sans"  # Can also use "Arial", "Calibri", "Helvetica"
    plt.rcParams.update({
    ...
    plt.tight_layout()
    plt.savefig('count_subs_modern_fonts.png', dpi=340, transparent=True)
    plt.show()
